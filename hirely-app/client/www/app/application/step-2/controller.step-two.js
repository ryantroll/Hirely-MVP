...
(function () {
  ...
  var hirelyApp = angular.module('hirelyApp').controller('StepTwoController', ['$scope', '$stateParams', '$filter', 'GeocodeService', 'OccupationService', 'AuthService', 'UserService', StepTwoController]);
    ...
    return {
      ...
      link:function(scope, ele, attrs, ctrl){
        ctrl.$parsers.unshift(function(value){
          var m = Number(value);
          var now = new Date();
          var valid = undefined !== m && NaN != m && m <= now.getFullYear();
  ...
  function StepTwoController($scope, $stateParams, $filter, GeocodeService, OccupationService, AuthService, UserService) {
      ...
      $scope.$on('$destroy', function(){
        ...
        .then(
          function(isAuth){
             ...
             */
            if(isAuth){
              var toSave = {workExperience:angular.copy($scope.xpItems)};
