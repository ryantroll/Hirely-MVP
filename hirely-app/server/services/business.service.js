...
var BusinessService = {
            ...
            function(found){
                ...
                try {
                    if (Array.isArray(found)) {
                         ...
                         * @type {Object}
                         */
                        return applicationModel.aggregate([{$match:{positionId:{$in:ids}, status:{$in:[2,3]}}}, {$group:{_id:'$positionId', applicationCount:{"$sum":1}}}]).exec()
    ...
    getPositionsByManagerId: function(managerId, reqQuery, isSuperUser){
        ...
        query.$or.push({srcId:managerId, srcType:'users', destType:'businesses', r:true});
        query.$or.push({srcId:managerId, srcType:'users', destType:'locations', r:true});
        query.$or.push({srcId:managerId, srcType:'users', destType:'positions', r:true});
        console.log('>>>>>>', query);
                ...
                if (found.length) {
                    ...
                    for (var i = 0; i < found.length; i++) {
                        ...
                        switch (found[i].destType) {
                            case 'positions':
                                var q = {};
                                q['positions.' + found[i].destId] = {$exists: true};
                                queryNext.$or.push(q);
