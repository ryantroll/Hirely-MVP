...
var traitifySevice = {
  ...
  getAssessmentCareerMatchesById: function (assessmentId) {
  ...
  updateAssessmentCareerMatchesByUserId: function (userId) {
        return userModel.findById(userId).then(function(user) {
           return traitifySevice.getAssessmentCareerMatchesById(user.personalityExams[0].extId).then(
               function(careerMatches) {
                   user.personalityExams[0].careerMatches = careerMatches;
                   return user.save();
               }
           )
        });
