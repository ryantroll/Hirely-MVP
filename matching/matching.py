  ...
  def updateUserCacheFields(user):
    ...
    for workExperience in user.workExperiences:
      if workExperience.onetOccupationId in roles:
        ...
        roles[workExperience.onetOccupationId] = {"experienceLvl": workExperience.experienceLvl}
?
    # add KSA calcs to roles - experienceLvl should equal the number of months in role
    for onetOccupationId, meta in roles.iteritems():
        roles[onetOccupationId]["knowledges"] = db.knowledges(onetOccupationId=onetOccupationId, workExperienceLowerBound<=meta["experienceLvl"], workExperienceUpperBound>meta["experienceLvl"])
        roles[onetOccupationId]["skills"] = db.skills(onetOccupationId=onetOccupationId, workExperienceLowerBound<=meta["experienceLvl"], workExperienceUpperBound>meta["experienceLvl"])
        roles[onetOccupationId]["abilities"] = db.abilities(onetOccupationId=onetOccupationId, workExperienceLowerBound<=meta["experienceLvl"], workExperienceUpperBound>meta["experienceLvl"])
        roles[onetOccupationId]["workActvities"] = db.workActivities(onetOccupationId=onetOccupationId, workExperienceLowerBound<=meta["experienceLvl"], workExperienceUpperBound>meta["experienceLvl"])
    ...
    user.workActivities = role1["workActvities"]
    weightingTotal = sum(workExperience.experienceLvl)
    ...
    for role in roles:
        ...
        for workActivityName, value in role["workActvities"]:
            value = float(role["experienceLvl"]/weightingTotal)*value
...
class MatchCacheService(object)
  class MatchCacheGenerator(object):
    ...
    onetOccupationIds = [...]
    matchScores = {}
    explevels =  list(set(db.knowledge.workExpereinceName))?
    maxScore = 0
    knowledgeWeight = .1
    skillWeight = .1
    abilityWeight = .1
...
?
    def generateForId(self, onetOccupationId):
      onet = onetOccupationId
    #You need to rememebr that Role experienceLvl and this Match score experienceLvl is 1 of 11 categoricals
      for experienceLvl in explevels:
        for scores in user.scores:
    ?
            # Knowledge calcs
            ss_user_knowledge = {}
            ss_job_knowledge = {}
            for knowledgeName, score in user.knowledges:  
              ss_user_knowledge[knowledgeName] = (user.knowledges[knowledgeName] - Knowledges[onet][experienceLvl][knowledgeName])**2 
              ss_job_knowledge[knowledgeName] =  Knowledges.get(onetOccupationId=onet,workExpereinceName=experienceLvl,elementComponenet=knowledgeName)**2
    ...
    ?
            ...
            for skillName, score in user.skills:
              ss_user_skill[skillName] = (user.skills[skillName] - Skills[onet][experienceLvl][skillName])**2 
              ss_job_skill[skillName] =  Skills.get(onetOccupationId=onet,workExpereinceName=experienceLvl,elementComponenet=skillName)**2
            ...
            for abilitiesName, score in user.abilities:
              ss_user_abilities[abilitiesName] = (user.abilities[abilitiesName] - Abilities[onet][experienceLvl][abilitiesName])**2 
              ss_job_abilities[abilitiesName] =  Abilities.get(onetOccupationId=onet,workExpereinceName=experienceLvl,elementComponenet=Abilities)**2
            ...
            for workActivitiesName, score in user.workActivities:
              ss_user_workActivities[workActivitiesName] = (user.workActivities[workActivitiesName] - WorkActivities[onet][experienceLvl][workActivitiesName])**2 
              ss_job_workActivities[workActivities] =  WorkActivities.get(onetOccupationId=onet,workExpereinceName=experienceLvl,elementComponenet=workActivities)**2
